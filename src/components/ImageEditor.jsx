...
import { removeWatermark, checkTaskStatus } from '../services/api'
...
const ImageEditor = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 发送去水印请求
      const response = await removeWatermark(imageUrl, apiKey)
      ...
      <form onSubmit={handleSubmit} className="mb-8 space-y-4">
        ...
        <div className="flex space-x-4">
          <button
            ...
            disabled={status === 'loading' || status === 'polling'}
          >
            {status === 'loading' ? '处理中...' : status === 'polling' ? '处理中...' : '去除水印'}
          </button>
      ...
      {resultImage && (
        <div className="mt-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              <h3 className="text-lg font-medium mb-2">去水印后</h3>
